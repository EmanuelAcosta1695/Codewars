function solve(s: string): string {
  let count_lowercase: number = (s.match(/[a-z]/g) || [])?.length
  let count_uppercase: number = (s.match(/[A-Z]/g) || [])?.length

  return count_uppercase > count_lowercase ? s.toUpperCase() : s.toLowerCase()
}
