export function findEvenIndex(arr: number[]): number {
    ...
    for (let i = 0; i <= arr.length; i++) {
        let resultA = i === 0 ? 0 : arr.slice(0, i).reduce((sum, current) => sum + current, 0)
        let resultB = i === arr.length ? 0 : arr.slice(i+1).reduce((sum, current) => sum + current, 0)

        if (resultA === resultB) {
            if (lowestSum === -1 || resultA < lowestSum) {
                lowestSum = resultA  
                indexResult = i
            } 
        }
...
// findEvenIndex([-1,1,0,-1,1,0,-1,1,0,0]) // 2. Right now return 0
